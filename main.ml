...
let print_command2 (q: Table.t option) (d: Database.t) (e: expr) (b: bool) =
  ...
  | Select (lst, tbl, w) -> (match q with
                            ...
                            | None -> Printf.printf "Error: Select failed. Table %s not found." tbl)
  ...
  | CreateTable(str, cdl) -> (if b then Printf.printf "Table %s created." str
                             else Printf.printf "Error: Create table failed. Table %s already exists." str)
  | DropTable(str) -> (if b then Printf.printf "Table %s dropped." str
                      else Printf.printf "Error: Drop table failed. Table %s not found." str)
  | _ -> failwith "Invalid command."

let print_command1 (e: expr) (b: bool) =
  match e with
  | CreateDb(str) -> (if b then (Printf.printf "Database with name %s created." str)
                     else (Printf.printf "Error: Create database failed. Database %s already exists." str))
  | DropDb(str) -> (if b then (Printf.printf "Database with name %s dropped." str)
                   else (Printf.printf "Error: Drop database failed. Database %s not found." str))
  | Use(str) -> (if b then (Printf.printf "Entered database with name %s." str)
                else (Printf.printf "Error: Database %s not found." str))
  | _ -> failwith "Invalid command."
